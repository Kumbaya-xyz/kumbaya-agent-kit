...
test("fire lifecycle: ignite a token (CREATE2 predicted == deployed), buy it, claim creator fees", { skip }, async () => {
  const sym = "T" + String(Date.now()).slice(-6);
  const ig: any = await wh("ignite")({ name: `MCP ${sym}`, symbol: sym, chainId: 6343 });
  assert.equal(ig.status, "success", "ignite confirmed");
  assert.equal(ig.predictedMatches, true, "mined CREATE2 address matches deployed token");
  assert.match(ig.token, /^0x[0-9a-fA-F]{40}$/);
  assert.ok(ig.pool, "pool address returned");

  // Freshly launched token is immediately tradeable via the router.
  const buy: any = await wh("swap")({ tokenIn: WETH, tokenOut: ig.token, amountIn: "0.001", chainId: 6343 });
  assert.equal(buy.status, "success", "buy of the new token confirmed");

  // Creator claims curve fees generated by the buy.
  const claim: any = await wh("claim_fees")({ token: ig.token, chainId: 6343 });
  assert.equal(claim.status, "success", "creator fee claim confirmed");
});

test("fire guards: release_vested and withdraw_tips reject cleanly when there is nothing to take", { skip }, async () => {
  // WETH is not a Fire token / not owed to us -> both must reject with a clear message, not send a tx.
  await assert.rejects(() => wh("release_vested")({ token: WETH, chainId: 6343 }) as any, /vested|beneficiary|revert/i);
  await assert.rejects(() => wh("withdraw_tips")({ token: WETH, chainId: 6343 }) as any, /withdrawable|Nothing/i);
});
